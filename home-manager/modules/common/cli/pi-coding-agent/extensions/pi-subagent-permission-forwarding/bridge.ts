...
export const BRIDGE_DELEGATE_ENV = "PI_PERMISSION_FORWARDING_DELEGATE_PI_BINARY";
...
export function deactivateInheritedBridgeEnvironment(env: NodeJS.ProcessEnv, wrapperPath: string): void {
  ...
  const bridgeOwnsBinary = configuredBinary
    ? path.isAbsolute(configuredBinary) && path.resolve(configuredBinary) === path.resolve(wrapperPath)
    : false;
  if (bridgeOwnsBinary) {
...
export function validateLaunchRecipe(options: Pick<BridgeInstallOptions, "wrapperPath" | "nodePath" | "cliPath">): void {
  const paths = {
  ...
  if (path.resolve(options.wrapperPath) === path.resolve(options.nodePath)) {
    throw new Error("The bridge wrapper and Node executable must be different paths.");
  }
}

export function installBridgeEnvironment(options: BridgeInstallOptions): BridgeInstallation {
  const sessionId = normalizeSessionId(options.sessionId);
  if (!sessionId) throw new Error("The interactive Pi session has no valid session ID.");
  ...
  const existingIsWrapper = existingBinary
    ? path.isAbsolute(existingBinary) && path.resolve(existingBinary) === path.resolve(options.wrapperPath)
    : false;
  const delegate = existingBinary && !existingIsWrapper
    ? existingBinary
    : env[BRIDGE_DELEGATE_ENV]?.trim();
