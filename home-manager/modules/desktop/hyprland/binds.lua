...
for _, entry in ipairs({
  ...
  bind(main_mod .. " + ALT + " .. entry[1], hl.dsp.window.resize({ x = entry[2], y = entry[3], relative = true }), repeating)
end
...
exec(main_mod .. " + G", scripts .. "/rofi.sh games")
exec(main_mod .. " + ALT + G", scripts .. "/gamemode.sh")
...
for _, entry in ipairs({
  { "right", "r+1" }, { "left", "r-1" }, { "h", "r-1" }, { "l", "r+1" },
  { "down", "empty" }, { "j", "empty" },
}) do
